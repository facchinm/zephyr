...
#define LOG_MODULE_NAME phy_mc_ksz9131
#define LOG_LEVEL CONFIG_PHY_LOG_LEVEL
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(LOG_MODULE_NAME);

#define MICROCHIP_OUI_MSB (0x22U)

#define PHY_MC_KSZ9131_CONTROL1_REG (0x1F)

#define PHY_MC_KSZ9131_BASIC_LINKSTATUS_MASK BIT(5)

#define PHY_MC_KSZ9131_CTL1_LINKDUPLEX_MASK BIT(3)
#define PHY_MC_KSZ9131_CTL1_LINKSPEED_MASK  (BIT(4) | BIT(5) | BIT(6))
#define PHY_MC_KSZ9131_CTL1_LINKSPEED_SHIFT (4U)
#define PHY_MC_KSZ9131_CTL1_LINKSPEED_10M   (1U)
#define PHY_MC_KSZ9131_CTL1_LINKSPEED_100M  (2U)
#define PHY_MC_KSZ9131_CTL1_LINKSPEED_1000M (4U)

#define PHY_MC_KSZ9131_INTERRUPT_REG        (0x1BU)
#define PHY_MC_KSZ9131_INTERRUPT_LINK       (BIT(8) | BIT(10))

#define PHY_MC_KSZ9031_WAKE_ON_LAN_CONTROL_MMD  (0x2U)
#define PHY_MC_KSZ9031_WAKE_ON_LAN_CONTROL_REG  (0x10U)

#define PHY_MC_KSZ9131_RESET_HOLD_TIME_MS 10

struct mc_ksz9131_config {
	uint8_t addr;
...
static int phy_mc_ksz9131_read(const struct device *dev,
				uint16_t reg_addr, uint32_t *data)
{
	const struct mc_ksz9131_config *config = dev->config;
	int ret;
...
static int phy_mc_ksz9131_write(const struct device *dev,
				uint16_t reg_addr, uint32_t data)
{
	const struct mc_ksz9131_config *config = dev->config;
	int ret;
...
static int phy_mc_ksz9131_reset(const struct device *dev)
{
	const struct mc_ksz9131_config *config = dev->config;
	uint32_t reg_val;
	int ret;
	...
	if (config->reset_gpio.port) {
		...
		k_busy_wait(USEC_PER_MSEC * PHY_MC_KSZ9131_RESET_HOLD_TIME_MS);
	...
	ret = phy_mc_ksz9131_write(dev, MII_BMCR, MII_BMCR_RESET);
	if (ret) {
	...
	do {
		ret = phy_mc_ksz9131_read(dev, MII_BMCR, &reg_val);
		if (ret) {
...
finalize_reset:
	...
	do {
		ret = phy_mc_ksz9131_read(dev, MII_PHYID1R, &reg_val);
		if (ret) {
			LOG_ERR("Error reading phy (%d) identifier register 1", config->addr);
			return ret;
		}
	} while (reg_val != MICROCHIP_OUI_MSB);

	return 0;
}

static int phy_mc_ksz9131_restart_autonegotiation(const struct device *dev)
{
	const struct mc_ksz9131_config *config = dev->config;
	uint32_t bmcr = 0;
	int ret;
	...
	ret = phy_mc_ksz9131_read(dev, MII_BMCR, &bmcr);
	if (ret) {
		LOG_ERR("Error reading phy (%d) basic control register", config->addr);
		return ret;
	}
	...
	ret = phy_mc_ksz9131_write(dev, MII_BMCR, bmcr);
	if (ret) {
		LOG_ERR("Error writing phy (%d) basic control register", config->addr);
		return ret;
	}
...
static int phy_mc_ksz9131_get_link(const struct device *dev,
					struct phy_link_state *state)
{
	const struct mc_ksz9131_config *config = dev->config;
	struct mc_ksz9131_data *data = dev->data;
	int ret;
	uint32_t control = 0;
	uint32_t basic = 0;
	uint32_t duplex = 0;
	...
	ret = phy_mc_ksz9131_read(dev, MII_BMSR, &basic);
	if (ret) {
	...
	ret = phy_mc_ksz9131_read(dev, PHY_MC_KSZ9131_CONTROL1_REG, &control);
	if (ret) {
	...
	new_state.is_up = basic & PHY_MC_KSZ9131_BASIC_LINKSTATUS_MASK;

	if (!new_state.is_up) {
		goto result;
	}

	duplex = (control & PHY_MC_KSZ9131_CTL1_LINKDUPLEX_MASK);
	switch ((control & PHY_MC_KSZ9131_CTL1_LINKSPEED_MASK)
				>> PHY_MC_KSZ9131_CTL1_LINKSPEED_SHIFT) {
		case PHY_MC_KSZ9131_CTL1_LINKSPEED_100M:
			if (duplex) {
			...
			break;
		case PHY_MC_KSZ9131_CTL1_LINKSPEED_1000M:
			if (duplex) {
			...
			break;
		case PHY_MC_KSZ9131_CTL1_LINKSPEED_10M:
		default:
			if (duplex) {
...
static int phy_mc_ksz9131_cfg_link(const struct device *dev,
					enum phy_link_speed speeds)
{
	...
	ret = phy_mc_ksz9131_read(dev, MII_ANAR, &anar);
	if (ret) {
	...
	ret = phy_mc_ksz9131_read(dev, MII_1KTCR, &gbcr);
	if (ret) {
	...
	ret = phy_mc_ksz9131_write(dev, MII_ANAR, anar);
	if (ret) {
	...
	ret = phy_mc_ksz9131_write(dev, MII_1KTCR, gbcr);
	if (ret) {
	...
	/* (Re)start autonegotiation */
	ret = phy_mc_ksz9131_restart_autonegotiation(dev);
	if (ret) {
...
static int phy_mc_ksz9131_link_cb_set(const struct device *dev,
					phy_callback_t cb, void *user_data)
{
	...
	phy_mc_ksz9131_get_link(dev, &data->state);
...
#if DT_ANY_INST_HAS_PROP_STATUS_OKAY(int_gpios)
static int phy_mc_ksz9131_clear_interrupt(struct mc_ksz9131_data *data)
{
	...
	ret = phy_mc_ksz9131_read(dev, PHY_MC_KSZ9131_INTERRUPT_REG, &reg_val);
	if (ret) {
...
static void phy_mc_ksz9131_interrupt_handler(const struct device *port,
						struct gpio_callback *cb,
						gpio_port_pins_t pins)
{
	struct mc_ksz9131_data *data = CONTAINER_OF(cb, struct mc_ksz9131_data, gpio_callback);
	int ret;
...
static void phy_mc_ksz9131_monitor_work_handler(struct k_work *work)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct mc_ksz9131_data *data =
		CONTAINER_OF(dwork, struct mc_ksz9131_data, phy_monitor_work);
	const struct device *dev = data->dev;
#if DT_ANY_INST_HAS_PROP_STATUS_OKAY(int_gpios)
	const struct mc_ksz9131_config *config = dev->config;
#endif /* DT_ANY_INST_HAS_PROP_STATUS_OKAY(int_gpios) */
...
#if DT_ANY_INST_HAS_PROP_STATUS_OKAY(int_gpios)
	if (config->interrupt_gpio.port) {
		ret = phy_mc_ksz9131_clear_interrupt(data);
		if (ret) {
			return;
		}
	}
	...
	ret = phy_mc_ksz9131_get_link(dev, &state);
...
static int phy_mc_ksz9131_init(const struct device *dev)
{
	const struct mc_ksz9131_config *config = dev->config;
	struct mc_ksz9131_data *data = dev->data;
	int ret;
	...
	/* Reset PHY */
	ret = phy_mc_ksz9131_reset(dev);
	if (ret) {
		LOG_ERR("Failed to reset phy (%d)", config->addr);
		return ret;
	}

	k_work_init_delayable(&data->phy_monitor_work, phy_mc_ksz9131_monitor_work_handler);

#if DT_ANY_INST_HAS_PROP_STATUS_OKAY(int_gpios)
	if (!config->interrupt_gpio.port) {
		phy_mc_ksz9131_monitor_work_handler(&data->phy_monitor_work.work);
		goto skip_int_gpio;
	}
	...
	gpio_init_callback(&data->gpio_callback, phy_mc_ksz9131_interrupt_handler,
...
#if 0
	/* Configure pin 38 for INT functionality (ksz9031 only) */
	ret = phy_mc_ksz9131_write(dev, 0xD, PHY_MC_KSZ9031_WAKE_ON_LAN_CONTROL_MMD);
	if (ret) {
		LOG_ERR("Error writing phy (%d) interrupt register", config->addr);
		return ret;
	}
	ret = phy_mc_ksz9131_write(dev, 0xE, PHY_MC_KSZ9031_WAKE_ON_LAN_CONTROL_REG);
	if (ret) {
		LOG_ERR("Error writing phy (%d) interrupt register", config->addr);
		return ret;
	}
	ret = phy_mc_ksz9131_write(dev, 0xD, 0x4002);
	if (ret) {
		LOG_ERR("Error writing phy (%d) interrupt register", config->addr);
		return ret;
	}
	ret = phy_mc_ksz9131_write(dev, 0xE, (1 << 15) | 3);
	if (ret) {
		LOG_ERR("Error writing phy (%d) interrupt register", config->addr);
		return ret;
	}
	...
	ret = phy_mc_ksz9131_write(dev, 0xE, 2);
	if (ret) {
		LOG_ERR("Error writing phy (%d) interrupt register", config->addr);
		return ret;
	}
	...
	ret = phy_mc_ksz9131_write(dev, 0xE, 1 << 10);
	if (ret) {
		LOG_ERR("Error writing phy (%d) interrupt register", config->addr);
		return ret;
	}
	...
	/* Enable interrupts mode */
	ret = phy_mc_ksz9131_write(dev, PHY_MC_KSZ9131_INTERRUPT_REG, PHY_MC_KSZ9131_INTERRUPT_LINK);
	if (ret) {
		LOG_ERR("Error writing phy (%d) interrupt register", config->addr);
		return ret;
	}
...
static DEVICE_API(ethphy, rt_rtl8211f_phy_api) = {
	.get_link = phy_mc_ksz9131_get_link,
	.cfg_link = phy_mc_ksz9131_cfg_link,
	.link_cb_set = phy_mc_ksz9131_link_cb_set,
	.read = phy_mc_ksz9131_read,
	.write = phy_mc_ksz9131_write,
};
...
#define REALTEK_RTL8211F_INIT(n)						\
	static const struct mc_ksz9131_config rt_rtl8211f_##n##_config = {	\
		...
		INTERRUPT_GPIO(n)						\
										...
										\
	static struct mc_ksz9131_data rt_rtl8211f_##n##_data;			\
										\
	DEVICE_DT_INST_DEFINE(n, &phy_mc_ksz9131_init, NULL,			\
