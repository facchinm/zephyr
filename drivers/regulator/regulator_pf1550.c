...
static const struct linear_range buck12_range[] = {
	LINEAR_RANGE_INIT(1100000, 0, 0, 0),
	LINEAR_RANGE_INIT(1200000, 0, 1, 1),
	LINEAR_RANGE_INIT(1350000, 0, 2, 2),
	LINEAR_RANGE_INIT(1500000, 0, 3, 3),
	LINEAR_RANGE_INIT(1800000, 0, 4, 4),
	LINEAR_RANGE_INIT(2500000, 0, 5, 5),
	LINEAR_RANGE_INIT(3000000, 0, 6, 6),
	LINEAR_RANGE_INIT(3300000, 0, 7, 7),
};
...
static int regulator_pf1550_set_mode(const struct device *dev, regulator_mode_t mode)
{
	// TODO: support LOAD SWITCH mode
	// BIT 4 of LDO1/3_CTRL
	return -ENOTSUP;
}
					   ...
					   int32_t *volt_uv)
{
	...
	return linear_range_group_get_value(config->desc->uv_range, config->desc->uv_nranges, idx, volt_uv);
}

static int regulator_pf1550_set_buck_ldo_voltage(const struct device *dev, int32_t min_uv,
						   int32_t max_uv, const struct linear_range *range, const uint8_t nranges,
						   uint8_t vout_reg)
{
	const struct regulator_pf1550_config *config = dev->config;
	uint16_t idx;
...
static int regulator_pf1550_buck12_ldo123_get_voltage(const struct device *dev,
							const struct linear_range *range, const uint8_t nranges,
...
static int regulator_pf1550_get_voltage(const struct device *dev, int32_t *volt_uv)
{
	...
	return regulator_pf1550_buck12_ldo123_get_voltage(dev,
...
static int regulator_pf1550_set_voltage(const struct device *dev, int32_t min_uv, int32_t max_uv)
{
	...
	return regulator_pf1550_set_buck_ldo_voltage(dev,
						       min_uv,
						       max_uv,
						       config->desc->uv_range,
						       config->desc->uv_nranges,
						 ...
						 int32_t *current_ua)
{
	...
	return linear_range_group_get_value(config->desc->ua_range, config->desc->ua_nranges, idx, current_ua);
}

static int regulator_pf1550_set_current_limit(const struct device *dev,
						int32_t min_ua,
						int32_t max_ua)
{
	const struct regulator_pf1550_config *config = dev->config;
	...
	ret = linear_range_group_get_win_index(config->desc->ua_range, config->desc->ua_nranges, min_ua, max_ua, &idx);
	if (ret == -EINVAL) {
		return ret;
	}
...
static int regulator_pf1550_power_off(const struct device *dev)
{
	...
	return i2c_reg_update_byte_dt(&common_config->bus, PMIC_PWRCTRL3, PF1550_GOTO_SHIP_MASK, PF1550_GOTO_SHIP);
}
...
static const struct regulator_driver_api api = {
	...
	.set_mode = regulator_pf1550_set_mode,
...
#define REGULATOR_PF1550_DEFINE(node_id, id, child_name, _source)				\
	...
	DEVICE_DT_DEFINE(node_id, regulator_pf1550_init, NULL,				\
			 &regulator_pf1550_data_##id,						\
			 &regulator_pf1550_config_##id,					\
			 POST_KERNEL,								\
			 CONFIG_REGULATOR_NXP_PF1550_INIT_PRIORITY,				\
			 &api);
...
#define REGULATOR_PF1550_DEFINE_ALL(inst)							\
	static const struct regulator_pf1550_common_config common_config_##inst = {		\
		.bus = I2C_DT_SPEC_GET(DT_INST_PARENT(inst)),					\
												...
												\
	DEVICE_DT_INST_DEFINE(inst, regulator_pf1550_common_init,				\
			      NULL, NULL, &common_config_##inst, POST_KERNEL,			\
			      CONFIG_REGULATOR_NXP_PF1550_COMMON_INIT_PRIORITY,		\
			      &parent_api);							\
												\
