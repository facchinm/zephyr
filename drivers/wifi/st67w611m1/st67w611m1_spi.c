...
int st67_spi_send(struct st67_spi_pkt *pkt)
{
	...
	ret = k_fifo_alloc_put(&spi_context.tx_fifo, pkt);
	if (ret < 0) {
		LOG_ERR("Failed to alloc and put in fifo: %d", ret);
		return ret;
	}
...
static int st67_spi_process(void)
{
	...
	if (spi_pkt == NULL) { /* Nothing to say */
		...
		if (pkt_len + sizeof(struct spi_header) > sizeof(spi_buffer)) {
			ret = -EMSGSIZE;
			goto out;
		}
		...
		size_t copied =
			net_buf_linearize(spi_buffer + sizeof(struct spi_header),
...
int st67_spi_init(void)
{
	...
	if (!gpio_is_ready_dt(&st67_config.chip_select) || !gpio_is_ready_dt(&st67_config.ready) ||
	    !gpio_is_ready_dt(&st67_config.chip_en) || !spi_is_ready_dt(&st67_config.spi)) {
